...
select left(b_date, 4) y, substring(b_date, 6, 2) m, rows from content_stream 
    where content_ts is not null and content_code='stat_unsched'
order by 1 desc, 2 desc
]]
	, "/plugins/unsched/calendar"
	)
    end
    )
end
...
select user_id, descr, dev_login, area, hidden from users
    order by descr
		...
		, "/plugins/unsched/users"
	    )
	end
	if err == nil or err == false then
	    tb.types, err = func_execute(tran,
[[
select unsched_type_id, descr, hidden from unsched_types
    order by row_no, descr
]]
		, "/plugins/unsched/unsched_types"
	    )
	end
	if err == nil or err == false then
	    tb.content, err = func_execute(tran,
[[
select content_ts, content_type, content_compress, content_blob from content_get('stat_unsched', '', 
    "monthDate_First"('%y%-%m%-01')::date_t, "monthDate_Last"('%y%-%m%-01')::date_t)
]]
		, "/plugins/unsched/content"
		, {y = year, m = month}
	    )
	end
	return tb, err
...
local function personalize(sestb, data)
	...
	for i, v in ipairs(p.rows) do
		...
		if v.unsched_type_id ~= nil then idx_types[v.unsched_type_id] = 1; end
    ...
    p.types = core.reduce(data.types, 'unsched_type_id', idx_types)
...
function M.scripts(lang, permtb, sestb, params)
    ...
    table.insert(ar, '<script src="' .. V.static_prefix .. '/popup.unschedtypes.js"> </script>')
    table.insert(ar, '<script src="' .. V.static_prefix .. '/popup.users.js"> </script>')
    table.insert(ar, '<script src="' .. V.static_prefix .. '/plugins/unsched.js"> </script>')
