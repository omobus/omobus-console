...
select rc_id, descr, ka_type, hidden from retail_chains
    order by descr
		...
		, "/plugins/trainings/retail_chains"
	    )
	end
	if err == nil or err == false then
...
select training_type_id, descr, hidden from training_types
    order by row_no, descr
		...
		, "/plugins/trainings/training_types"
	    )
	end
	if err == nil or err == false then
...
select tm_id, descr, hidden from training_materials
    order by descr
		...
		, "/plugins/trainings/training_materials"
	    )
	end
	if permtb.channel == true and (err == nil or err == false) then
...
select activity_type_id, descr, hidden from activity_types
    order by descr
		...
		, "/plugins/trainings/activity_types"
	    )
	end
	if err == nil or err == false then
