...
[[
select left(b_date, 4) y, substring(b_date, 6, 2) m, rows from content_stream where content_ts is not null and content_code='stat_photos'
...
[[
select content_ts, content_type, content_compress, content_blob from content_get('stat_photos', '', 
...
local function personalize(sestb, data)
    ...
    -- build filters:
    local x = {u = {}, chan = {}, rc = {}, t = {}, p = {}, brand = {}, e = {}}
    for i, v in ipairs(p.rows) do
	x.u[v.user_id] = 1
	if v.chan_id ~= nil then x.chan[v.chan_id] = 1; end
	if v.rc_id ~= nil then x.rc[v.rc_id] = 1; end
	if v.photo_type_id ~= nil then x.t[v.photo_type_id] = 1; end
	if v.placement_id ~= nil then x.p[v.placement_id] = 1; end
	if v.brand_id ~= nil then x.brand[v.brand_id] = 1; end
    ...
    p.types = core.reduce(data.types, 'photo_type_id', x.t)
    p.placements = core.reduce(data.placements, 'placement_id', x.p)
    p.brands = core.reduce(data.brands, 'brand_id', x.brand)
...
function M.scripts(lang, permtb, sestb, params)
    ...
    table.insert(ar, '<script src="' .. V.static_prefix .. '/plugins/photos.js"> </script>')
...
function M.ajax(lang, method, permtb, sestb, params, content, content_type, stor, res)
	...
	if permtb.target == true then
	    ...
	    -- validate input data
	    assert(p.sub ~= nil and #p.sub, string.format("function %s() invalid target subject.", debug.getinfo(1,"n").name))
