...
[[
select left(b_date, 4) y, substring(b_date, 6, 2) m, rows from content_stream where content_ts is not null and content_code='stat_quests'
...
local function personalize(sestb, data)
    ...
    -- build filters:
    local x = {u = {}, chan = {}, rc = {}, n = {}, r = {}, e = {}}
    for i, v in ipairs(p.rows) do
	x.u[v.user_id] = 1
	if v.chan_id ~= nil then x.chan[v.chan_id] = 1; end
	if v.rc_id ~= nil then x.rc[v.rc_id] = 1; end
	if v.qname_id ~= nil then x.n[v.qname_id] = 1; end
	if v.qrow_id ~= nil then x.r[v.qrow_id] = 1; end
    ...
    p.qnames = core.reduce(data.qnames, 'qname_id', x.n)
    p.qrows = core.reduce(data.qrows, 'qrow_id', x.r)
