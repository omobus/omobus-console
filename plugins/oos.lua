...
select left(b_date, 4) y, substring(b_date, 6, 2) m, rows from content_stream 
    where content_ts is not null and content_code='stat_oos'
order by 1 desc, 2 desc
]]
	, "/plugins/oos/calendar"
	)
    end
    )
end
...
select categ_id, descr, hidden from categories
    order by descr
		...
		, "/plugins/oos/categories"
	    )
	end
	if err == nil or err == false then
	    tb.oos_types, err = func_execute(tran,
[[
select oos_type_id, descr, hidden from oos_types
    order by descr
]]
		, "/plugins/oos/oos_types"
	    )
	end
	if err == nil or err == false then
	    tb.content, err = func_execute(tran,
[[
select content_ts, content_type, content_compress, content_blob from content_get('stat_oos', '', 
...
local function personalize(sestb, data)
	...
	for i, v in ipairs(p.rows) do
		...
		if v.oos_type_id ~= nil then idx_types[v.oos_type_id] = 1; end
    ...
    p.oos_types = core.reduce(data.oos_types, 'oos_type_id', idx_types)
...
function M.scripts(lang, permtb, sestb, params)
    ...
    table.insert(ar, '<script src="' .. V.static_prefix .. '/popup.oostypes.js"> </script>')
    table.insert(ar, '<script src="' .. V.static_prefix .. '/popup.users.js"> </script>')
    table.insert(ar, '<script src="' .. V.static_prefix .. '/plugins/oos.js"> </script>')
