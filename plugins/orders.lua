...
[[
select left(b_date, 4) y, substring(b_date, 6, 2) m, rows from content_stream where content_ts is not null and content_code='stat_orders'
...
local function personalize(sestb, data)
	...
	if v.order_type_id ~= nil then x.t[v.order_type_id] = 1; end
	if v.distr_id ~= nil then x.d[v.distr_id] = 1; end
	if v.delivery_type_id ~= nil then x.s[v.delivery_type_id] = 1; end
	if v.payment_method_id ~= nil then x.p[v.payment_method_id] = 1; end
    end
    ...
    p.order_types = core.reduce(data.order_types, 'order_type_id', x.t)
    p.distributors = core.reduce(data.distributors, 'distr_id', x.d)
    p.delivery_types = core.reduce(data.delivery_types, 'delivery_type_id', x.s)
    p.payment_methods = core.reduce(data.payment_methods, 'payment_method_id', x.p)
