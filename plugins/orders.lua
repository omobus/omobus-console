...
select left(b_date, 4) y, substring(b_date, 6, 2) m, rows from content_stream 
    where content_ts is not null and content_code='stat_orders'
order by 1 desc, 2 desc
]]
	, "/plugins/orders/calendar"
	)
    end
    )
end
...
select rc_id, descr, ka_type, hidden from retail_chains
    order by descr
		...
		, "/plugins/orders/retail_chains"
	    )
	end
	if err == nil or err == false then
	    tb.order_types, err = func_execute(tran,
[[
select order_type_id, descr, hidden from order_types
    order by descr
]]
		, "/plugins/orders/order_types"
	    )
	end
	if err == nil or err == false then
	    tb.distributors, err = func_execute(tran,
[[
select distr_id, descr, hidden from distributors
    order by descr
]]
		, "/plugins/orders/distributors"
	    )
	end
	if err == nil or err == false then
	    tb.delivery_types, err = func_execute(tran,
[[
select delivery_type_id, descr, hidden from delivery_types
    order by descr
]]
		, "/plugins/orders/delivery_types"
	    )
	end
	if err == nil or err == false then
	    tb.payment_methods, err = func_execute(tran,
[[
select payment_method_id, descr, hidden from payment_methods
    order by descr
]]
		, "/plugins/orders/payment_methods"
	    )
	end
	if err == nil or err == false then
	    tb.content, err = func_execute(tran,
[[
select content_ts, content_type, content_compress, content_blob from content_get('stat_orders', '', 
    "monthDate_First"('%y%-%m%-01')::date_t, "monthDate_Last"('%y%-%m%-01')::date_t)
]]
		, "/plugins/orders/content"
		, {y = year, m = month}
	    )
	end
	return tb, err
...
local function personalize(sestb, data)
	...
	for i, v in ipairs(p.rows) do
		...
		if v.order_type_id ~= nil then idx_types[v.order_type_id] = 1; end
		if v.delivery_type_id ~= nil then idx_deliveries[v.delivery_type_id] = 1; end
		if v.payment_method_id ~= nil then idx_payments[v.payment_method_id] = 1; end
    ...
    p.order_types = core.reduce(data.order_types, 'order_type_id', idx_types)
    p.distributors = core.reduce(data.distributors, 'distr_id', idx_distributors)
    p.delivery_types = core.reduce(data.delivery_types, 'delivery_type_id', idx_deliveries)
    p.payment_methods = core.reduce(data.payment_methods, 'payment_method_id', idx_payments)
...
function M.scripts(lang, permtb, sestb, params)
    ...
    table.insert(ar, '<script src="' .. V.static_prefix .. '/popup.deliverytypes.js"> </script>')
    table.insert(ar, '<script src="' .. V.static_prefix .. '/popup.distributors.js"> </script>')
    table.insert(ar, '<script src="' .. V.static_prefix .. '/popup.ordertypes.js"> </script>')
    table.insert(ar, '<script src="' .. V.static_prefix .. '/popup.paymentmethods.js"> </script>')
    ...
    table.insert(ar, '<script src="' .. V.static_prefix .. '/plugins/orders.js"> </script>')
