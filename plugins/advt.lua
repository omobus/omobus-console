...
[[
select left(b_date, 4) y, substring(b_date, 6, 2) m, rows from content_stream where content_ts is not null and content_code='stat_advt'
...
local function personalize(sestb, data)
	...
	if v.posm_id ~= nil then x.m[v.posm_id] = 1; end
    ...
    p.posms = core.reduce(data.posms, 'posm_id', x.m)
...
function M.scripts(lang, permtb, sestb, params)
    ...
    table.insert(ar, '<script src="' .. V.static_prefix .. '/plugins/advt.js"> </script>')
