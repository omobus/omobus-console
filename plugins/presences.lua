...
select left(b_date, 4) y, substring(b_date, 6, 2) m, rows from content_stream 
    where content_ts is not null and content_code='stat_presences'
order by 1 desc, 2 desc
]]
	, "/plugins/presences/calendar"
	)
    end
    )
end
...
select rc_id, descr, ka_type, hidden from retail_chains
    order by descr
		...
		, "/plugins/presences/retail_chains"
	    )
	end
...
[[
select content_ts, content_type, content_compress, content_blob from content_get('stat_presences', '', 
    "monthDate_First"('%y%-%m%-01')::date_t, "monthDate_Last"('%y%-%m%-01')::date_t)
]]
		, "/plugins/presences/content"
		, {y = year, m = month}
	    )
	end
	return tb, err
...
function M.scripts(lang, permtb, sestb, params)
    ...
    table.insert(ar, '<script src="' .. V.static_prefix .. '/plugins/presences.js"> </script>')
