...
select left(b_date, 4) y, substring(b_date, 6, 2) m, rows from content_stream 
    where content_ts is not null and content_code='stat_comments'
order by 1 desc, 2 desc
]]
	, "/plugins/comments/calendar"
	)
    end
    )
end
...
select rc_id, descr, ka_type, hidden from retail_chains
    order by descr
		...
		, "/plugins/comments/retail_chains"
	    )
	end
	if err == nil or err == false then
	    tb.types, err = func_execute(tran,
[[
select comment_type_id, descr, hidden from comment_types
    order by row_no, descr
]]
		, "/plugins/comments/comment_types"
	    )
	end
	if err == nil or err == false then
	    tb.content, err = func_execute(tran,
[[
select content_ts, content_type, content_compress, content_blob from content_get('stat_comments', '', 
    "monthDate_First"('%y%-%m%-01')::date_t, "monthDate_Last"('%y%-%m%-01')::date_t)
]]
		, "/plugins/comments/content"
		, {y = year, m = month}
	    )
	end
	return tb, err
...
local function personalize(sestb, data)
    ...
    p.types = core.reduce(data.types, 'comment_type_id', idx_types)
...
function M.scripts(lang, permtb, sestb, params)
    ...
    table.insert(ar, '<script src="' .. V.static_prefix .. '/popup.commenttypes.js"> </script>')
    ...
    table.insert(ar, '<script src="' .. V.static_prefix .. '/plugins/comments.js"> </script>')
