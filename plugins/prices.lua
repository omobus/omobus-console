...
[[
select left(b_date, 4) y, substring(b_date, 6, 2) m, rows from content_stream where content_ts is not null and content_code='stat_prices'
...
[[
select content_ts, content_type, content_compress, content_blob from content_get('stat_prices', '', 
...
local function personalize(sestb, data)
	...
	if v.categ_id ~= nil then x.c[v.categ_id] = 1; end
    ...
    p.categories = core.reduce(data.categories, 'categ_id', x.c)
...
function M.scripts(lang, permtb, sestb, params)
    ...
    table.insert(ar, '<script src="' .. V.static_prefix .. '/plugins/prices.js"> </script>')
