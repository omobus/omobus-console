...
[[
select 
    m.pl_id, m.descr, m.country_id, c.descr country, m.brand_ids[1] brand_id, b.descr brand, b.dep_id, d.descr department, blob_length(m.image) size, 
    array_to_string(m.chan_ids, ',') chan_ids, (select string_agg(descr, '<br/>') from channels where chan_id=any(m.chan_ids)) channels,
...
local function get_blob(stor, pl_id)
    return stor.get(function(tran, func_execute) return func_execute(tran,
-- *** sql query: begin
[[
select pl_id, image from planograms 
    where pl_id=%pl_id%
]]
-- *** sql query: end
        , "//planograms/image/"
...
local function get_author(stor, pl_id)
    return stor.get(function(tran, func_execute) return func_execute(tran,
-- *** sql query: begin
[[
select author_id from planograms 
    where pl_id=%pl_id%
]]
-- *** sql query: end
	, "//planograms/author/"
...
local function remove(stor, uid, pl_id)
...
-- *** sql query: begin
[[
insert into planograms(descr, image, country_id, brand_ids, chan_ids, dep_id, author_id)
    values(%name%, %1:blob%, %country_id%, string_to_array(%brand_id%,','), string_to_array(%chan_ids%,','), (select dep_id from brands where brand_id=%brand_id%), %req_uid%)
]]
-- *** sql query: end
        , "//planograms/new/"
        , params
...
-- *** sql query: begin
[[
update planograms set descr=%name%, country_id=%country_id%, brand_ids=string_to_array(%brand_id%,','), chan_ids=string_to_array(%chan_ids%,','), dep_id=(select dep_id from brands where brand_id=%brand_id%)
    where pl_id=%pl_id%
]]
-- *** sql query: end
        , "//planograms/edit/"
...
local function ajax_remove(permtb, sestb, params, stor, res)
    -- validate input data
    assert(validate.isuid(params.pl_id), 
	string.format("function %s() >>> PANIC <<< invalid pl_id.", debug.getinfo(1,"n").name))
    -- check target owner
    if permtb.remove == nil or permtb.remove ~= true then
	local tb, err = get_author(stor, params.pl_id)
	assert(tb ~= nil and #tb == 1 and tb[1].author_id == iif(sestb.erpid ~= nil, sestb.erpid, sestb.username),
	    string.format("function %s() >>> PANIC <<< unable to remove planogramm created by %s.",
	    debug.getinfo(1,"n").name, iif(tb[1].author_id ~= nil, tb[1].author_id, '-')))
    end
    -- execute query
    if remove(stor, iif(sestb.erpid ~= nil, sestb.erpid, sestb.username), params.pl_id) then
	scgi.writeHeader(res, 500, {["Content-Type"] = mime.txt .. "; charset=utf-8"})
	scgi.writeBody(res, "Internal server error")
    else
...
local function ajax_new(permtb, sestb, params, tb, stor, res)
    ...
    if tb.chan_ids == nil or #tb.chan_ids == 0 then tb.chan_ids = stor.NULL end
    -- execute query
...
local function ajax_edit(permtb, sestb, params, tb, stor, res)
	...
	local tb, err = get_author(stor, params.pl_id)
	assert(tb ~= nil and #tb == 1 and tb[1].author_id == iif(sestb.erpid ~= nil, sestb.erpid, sestb.username),
	    string.format("function %s() >>> PANIC <<< unable to edit planogramm created by %s.",
	    debug.getinfo(1,"n").name, iif(tb[1].author_id ~= nil, tb[1].author_id, '-')))
    end
    -- set unknown values
    if tb.country_id == nil or #tb.country_id == 0 then tb.country_id = stor.NULL end
...
local function ajax_blob(params, stor, res)
	...
	string.format("function %s() >>> PANIC <<< invalid pl_id.", debug.getinfo(1,"n").name))
    -- execute query
    local tb, err = get_blob(stor, params.pl_id)
    if err then
	scgi.writeHeader(res, 500, {["Content-Type"] = mime.txt .. "; charset=utf-8"})
	scgi.writeBody(res, "Internal server error")
...
function M.scripts(lang, permtb, sestb, params)
    ...
    <script src="]] .. V.static_prefix .. [[/plugins/planograms.js"> </script>
]]
end
