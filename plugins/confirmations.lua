...
order by 1 desc, 2 desc
	...
	, "/plugins/confirmations/calendar"
	)
    end
    )
end
...
select account_id from (select expand_cities(city_id) city_id, chan_id from my_cities where user_id in (select my_staff(%user_id%, 1::bool_t))) c, accounts a where c.city_id=a.city_id and (c.chan_id='' or c.chan_id=a.chan_id)
		    ...
		    , "/plugins/confirmations/F.accounts"
		    , {user_id = sestb.erpid}
		)
	    end
...
select chan_id, descr, hidden from channels
    order by descr
		...
		, "/plugins/confirmations/channels"
	    )
	end
	if err == nil or err == false then
...
select rc_id, descr, ka_type, hidden from retail_chains
    order by descr
		...
		, "/plugins/confirmations/retail_chains"
	    )
	end
	if err == nil or err == false then
...
select confirmation_type_id, descr, hidden from confirmation_types
    order by descr
		...
		, "/plugins/confirmations/confirmation_types"
	    )
	end
	if err == nil or err == false then
...
[[
select doc_id, status, note from j_remarks
    where doc_id in (
...
select thumb_get(%blob_id%::blob_t) photo
	...
	, "/plugins/confirmations/thumb"
	, {blob_id = blob_id})
    end
    )
end

local function remark(stor, uid, reqdt, cmd, doc_id, note)
    return stor.get(function(tran, func_execute) return func_execute(tran,
[[
select console.req_remark(%req_uid%, %req_dt%, %cmd%, %doc_id%, %msg%) "rows"
]]
	, "/plugins/confirmations/remark"
	, {req_uid = uid, req_dt = reqdt, cmd = cmd, doc_id = doc_id, msg = note or stor.NULL})
    end, false
    )
end
...
function M.data(lang, method, permtb, sestb, params, content, content_type, stor, res)
	...
	if permtb.remark == true then
	    ...
	    tb, err = remark(stor, sestb.erpid or sestb.username, p._datetime, 'accept', p.doc_id, p.note)
	    if err then
	...
	if permtb.remark == true then
	    ...
	    assert(validate.isdatetime(p._datetime), "invalid [_datetime] parameter.")
	    assert(validate.isuid(p.doc_id), "invalid [doc_id] parameter.")
	    assert(p.note ~= nil and #p.note, "missing required parameters [note].")
	    tb, err = remark(stor, sestb.erpid or sestb.username, p._datetime, 'reject', p.doc_id, p.note)
	    if err then
		scgi.writeHeader(res, 500, {["Content-Type"] = mime.json .. "; charset=utf-8"})
		scgi.writeBody(res, "{\"status\":\"failed\",\"msg\":\"Internal server error\"}")
	    elseif tb == nil or #tb ~= 1 or tb[1].rows == nil or tb[1].rows == 0 then
