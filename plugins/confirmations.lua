...
[[
select left(b_date, 4) y, substring(b_date, 6, 2) m, rows from content_stream where content_ts is not null and content_code='stat_confirmations'
...
local function personalize(sestb, data)
    ...
    -- build filters:
    local x = {u = {}, chan = {}, rc = {}, t = {}, a = {}, e = {}}
    for i, v in ipairs(p.rows) do
	x.u[v.user_id] = 1
	if v.chan_id ~= nil then x.chan[v.chan_id] = 1; end
	if v.rc_id ~= nil then x.rc[v.rc_id] = 1; end
	if v.confirm_id ~= nil then x.t[v.confirm_id] = 1; end
	if v.author_id ~= nil then x.a[v.author_id] = 1; end
