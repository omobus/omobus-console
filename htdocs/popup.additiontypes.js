...
function AdditionTypesPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof AdditionTypesPopup) ) {
	return new AdditionTypesPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("additiontypesPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("additiontypesPopup");
    }
	...
	if( arg.getAttribute('name') == 'additiontypesEverything' ) {
...
(function (AdditionTypesPopup, undefined) {
    AdditionTypesPopup.container = function(id) {
	return "<div id='" + (id == null || typeof id == 'undefined' ? "additiontypesPopup" : id) + 
...
AdditionTypesPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='additiontypesEverything' href='javascript:void(0);'>", lang.additions.everything, "</a></div>");
    }
    return ar;
}

AdditionTypesPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

AdditionTypesPopup.prototype._onsearch = function(value) {
...
AdditionTypesPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].addition_type_id);
    }
}

AdditionTypesPopup.prototype._oneverything = function() {
...
AdditionTypesPopup.prototype._show = function(arg, offset) {
...
AdditionTypesPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

AdditionTypesPopup.prototype.hide = function() {
    this._container.hide();
}

AdditionTypesPopup.prototype.toggle = function(arg, offset) {
