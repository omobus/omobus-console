...
function ChannelsPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof ChannelsPopup) ) {
	return new ChannelsPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("channelsPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("channelsPopup");
    }
	...
	if( arg.getAttribute('name') == 'channelsEverything' ) {
...
(function (ChannelsPopup, undefined) {
    ChannelsPopup.container = function(id) {
	return "<div id='" + (id == null || typeof id == 'undefined' ? "channelsPopup" : id) + 
...
ChannelsPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='channelsEverything' href='javascript:void(0);'>", lang.chan_everything, "</a></div>");
    }
    return ar;
}

ChannelsPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

ChannelsPopup.prototype._onsearch = function(value) {
...
ChannelsPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].chan_id);
    }
}

ChannelsPopup.prototype._oneverything = function() {
...
ChannelsPopup.prototype._show = function(arg, offset) {
...
ChannelsPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

ChannelsPopup.prototype.hide = function() {
    this._container.hide();
}

ChannelsPopup.prototype.toggle = function(arg, offset) {
