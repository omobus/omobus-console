...
function POSMaterialsPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof POSMaterialsPopup) ) {
	return new POSMaterialsPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("posmsPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("posmsPopup");
    }
	...
	if( arg.getAttribute('name') == 'posmsEverything' ) {
...
(function (POSMaterialsPopup, undefined) {
    POSMaterialsPopup.container = function(id) {
	return "<div id='" + (id == null || typeof id == 'undefined' ? "posmsPopup" : id) + 
...
POSMaterialsPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='posmsEverything' href='javascript:void(0);'>", lang.pos_material_everything, "</a></div>");
    }
    return ar;
}

POSMaterialsPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

POSMaterialsPopup.prototype._onsearch = function(value) {
...
POSMaterialsPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].posm_id);
    }
}

POSMaterialsPopup.prototype._oneverything = function() {
...
POSMaterialsPopup.prototype._show = function(arg, offset) {
...
POSMaterialsPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

POSMaterialsPopup.prototype.hide = function() {
    this._container.hide();
}

POSMaterialsPopup.prototype.toggle = function(arg, offset) {
