...
function UnschedTypesPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof UnschedTypesPopup) ) {
	return new UnschedTypesPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("unschedtypesPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("unschedtypesPopup");
    }
	...
	if( arg.getAttribute('name') == 'unschedtypesEverything' ) {
...
UnschedTypesPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='unschedtypesEverything' href='javascript:void(0);'>", lang.unsched.everything, "</a></div>");
    }
    return ar;
}

UnschedTypesPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

UnschedTypesPopup.prototype._onsearch = function(value) {
...
UnschedTypesPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].unsched_type_id);
    }
}

UnschedTypesPopup.prototype._oneverything = function() {
...
UnschedTypesPopup.prototype._show = function(arg, offset) {
...
UnschedTypesPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

UnschedTypesPopup.prototype.hide = function() {
    this._container.hide();
}

UnschedTypesPopup.prototype.toggle = function(arg, offset) {
    if( this._container.toggle() ) {
	this._show(arg, offset);
    }
}

UnschedTypesPopup.prototype.isHidden = function() {
