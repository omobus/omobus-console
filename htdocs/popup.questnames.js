...
function QuestNamesPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof QuestNamesPopup) ) {
	return new QuestNamesPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("qnamesPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("qnamesPopup");
    }
	...
	if( arg.getAttribute('name') == 'qnamesEverything' ) {
...
(function (QuestNamesPopup, undefined) {
    QuestNamesPopup.container = function(id) {
	return "<div id='" + (id == null || typeof id == 'undefined' ? "qnamesPopup" : id) + 
...
QuestNamesPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='qnamesEverything' href='javascript:void(0);'>", lang.qname_everything, "</a></div>");
    }
    return ar;
}

QuestNamesPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

QuestNamesPopup.prototype._onsearch = function(value) {
...
QuestNamesPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].qname_id);
    }
}

QuestNamesPopup.prototype._oneverything = function() {
...
QuestNamesPopup.prototype._show = function(arg, offset) {
...
QuestNamesPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

QuestNamesPopup.prototype.hide = function() {
    this._container.hide();
}

QuestNamesPopup.prototype.toggle = function(arg, offset) {
