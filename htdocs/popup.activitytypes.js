...
function ActivityTypesPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof ActivityTypesPopup) ) {
	return new ActivityTypesPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("activitytypesPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("activitytypesPopup");
    }
	...
	if( arg.getAttribute('name') == 'activitytypesEverything' ) {
...
ActivityTypesPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='activitytypesEverything' href='javascript:void(0);'>", lang.activity_type_everything, "</a></div>");
    }
    return ar;
}

ActivityTypesPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

ActivityTypesPopup.prototype._onsearch = function(value) {
...
ActivityTypesPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].activity_type_id);
    }
}

ActivityTypesPopup.prototype._oneverything = function() {
...
ActivityTypesPopup.prototype._show = function(arg, offset) {
...
ActivityTypesPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

ActivityTypesPopup.prototype.hide = function() {
    this._container.hide();
}

ActivityTypesPopup.prototype.toggle = function(arg, offset) {
    if( this._container.toggle() ) {
	this._show(arg, offset);
    }
}

ActivityTypesPopup.prototype.isHidden = function() {
