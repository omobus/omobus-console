...
function ASPTypesPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof ASPTypesPopup) ) {
	return new ASPTypesPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("asptypesPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("asptypesPopup");
    }
	...
	if( arg.getAttribute('name') == 'asptypesEverything' ) {
...
(function (ASPTypesPopup, undefined) {
    ASPTypesPopup.container = function(id) {
	return "<div id='" + (id == null || typeof id == 'undefined' ? "asptypesPopup" : id) + 
...
ASPTypesPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='asptypesEverything' href='javascript:void(0);'>", lang.asp_type_everything, "</a></div>");
    }
    return ar;
}

ASPTypesPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

ASPTypesPopup.prototype._onsearch = function(value) {
...
ASPTypesPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].asp_type_id);
    }
}

ASPTypesPopup.prototype._oneverything = function() {
...
ASPTypesPopup.prototype._show = function(arg, offset) {
...
ASPTypesPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

ASPTypesPopup.prototype.hide = function() {
    this._container.hide();
}

ASPTypesPopup.prototype.toggle = function(arg, offset) {
    if( this._container.toggle() ) {
	this._show(arg, offset);
    }
}

ASPTypesPopup.prototype.isHidden = function() {
