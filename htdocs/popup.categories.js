...
function CategoriesPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof CategoriesPopup) ) {
	return new CategoriesPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("categoriesPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("categoriesPopup");
    }
	...
	if( arg.getAttribute('name') == 'categoriesEverything' ) {
...
CategoriesPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='categoriesEverything' href='javascript:void(0);'>", lang.categ_everything, "</a></div>");
    }
    return ar;
}

CategoriesPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

CategoriesPopup.prototype._onsearch = function(value) {
...
CategoriesPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].categ_id);
    }
}

CategoriesPopup.prototype._oneverything = function() {
...
CategoriesPopup.prototype._show = function(arg, offset) {
...
CategoriesPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

CategoriesPopup.prototype.hide = function() {
    this._container.hide();
}

CategoriesPopup.prototype.toggle = function(arg, offset) {
