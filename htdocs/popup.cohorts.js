...
function CohortsPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof CohortsPopup) ) {
	return new CohortsPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("cohortsPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("cohortsPopup");
    }
	...
	if( arg.getAttribute('name') == 'cohortsEverything' ) {
...
CohortsPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='cohortsEverything' href='javascript:void(0);'>", lang.cohort_everything, "</a></div>");
    }
    return ar;
}

CohortsPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

CohortsPopup.prototype._onsearch = function(value) {
...
CohortsPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].cohort_id);
    }
}

CohortsPopup.prototype._oneverything = function() {
...
CohortsPopup.prototype._show = function(arg, offset) {
...
CohortsPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

CohortsPopup.prototype.hide = function() {
    this._container.hide();
}

CohortsPopup.prototype.toggle = function(arg, offset) {
    if( this._container.toggle() ) {
	this._show(arg, offset);
    }
}

CohortsPopup.prototype.isHidden = function() {
