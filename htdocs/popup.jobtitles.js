...
function JobTitlesPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof JobTitlesPopup) ) {
	return new JobTitlesPopup(rows, selection, params);
    }
...
(function (JobTitlesPopup, undefined) {
    JobTitlesPopup.container = function(id) {
...
JobTitlesPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='jobtitlesEverything' href='javascript:void(0);'>", lang.job_title_everything, "</a></div>");
    }
    return ar;
}

JobTitlesPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

JobTitlesPopup.prototype._onsearch = function(value) {
...
JobTitlesPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].job_title_id);
    }
}

JobTitlesPopup.prototype._oneverything = function() {
...
JobTitlesPopup.prototype._show = function(arg, offset) {
...
JobTitlesPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

JobTitlesPopup.prototype.hide = function() {
    this._container.hide();
}

JobTitlesPopup.prototype.toggle = function(arg, offset) {
