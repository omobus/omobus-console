...
function TargetTypesPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof TargetTypesPopup) ) {
	return new TargetTypesPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("targettypesPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("targettypesPopup");
    }
	...
	if( arg.getAttribute('name') == 'targettypesEverything' ) {
...
(function (TargetTypesPopup, undefined) {
    TargetTypesPopup.container = function(id) {
	return "<div id='" + (id == null || typeof id == 'undefined' ? "targettypesPopup" : id) + 
...
TargetTypesPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='targettypesEverything' href='javascript:void(0);'>", lang.targets.everything, "</a></div>");
    }
    return ar;
}

TargetTypesPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

TargetTypesPopup.prototype._onsearch = function(value) {
...
TargetTypesPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].target_type_id);
    }
}

TargetTypesPopup.prototype._oneverything = function() {
...
TargetTypesPopup.prototype._show = function(arg, offset) {
...
TargetTypesPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

TargetTypesPopup.prototype.hide = function() {
    this._container.hide();
}

TargetTypesPopup.prototype.toggle = function(arg, offset) {
    if( this._container.toggle() ) {
	this._show(arg, offset);
    }
}

TargetTypesPopup.prototype.isHidden = function() {
