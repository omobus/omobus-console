...
function TrainingTypesPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
	...
	return new TrainingTypesPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("trainingtypesPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("trainingtypesPopup");
    }
	...
	if( arg.getAttribute('name') == 'trainingtypesEverything' ) {
...
(function (TrainingTypesPopup, undefined) {
    TrainingTypesPopup.container = function(id) {
	return "<div id='" + (id == null || typeof id == 'undefined' ? "trainingtypesPopup" : id) + 
...
TrainingTypesPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='trainingtypesEverything' href='javascript:void(0);'>", lang.training_type_everything, "</a></div>");
    }
    return ar;
}

TrainingTypesPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

TrainingTypesPopup.prototype._onsearch = function(value) {
...
TrainingTypesPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].training_type_id);
    }
}

TrainingTypesPopup.prototype._oneverything = function() {
...
TrainingTypesPopup.prototype._show = function(arg, offset) {
...
TrainingTypesPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

TrainingTypesPopup.prototype.hide = function() {
    this._container.hide();
}

TrainingTypesPopup.prototype.toggle = function(arg, offset) {
