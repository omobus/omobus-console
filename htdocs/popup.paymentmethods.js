...
function PaymentMethodsPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof PaymentMethodsPopup) ) {
	return new PaymentMethodsPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("paymentmethodsPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("paymentmethodsPopup");
    }
	...
	if( arg.getAttribute('name') == 'paymentmethodsEverything' ) {
...
(function (PaymentMethodsPopup, undefined) {
    PaymentMethodsPopup.container = function(id) {
	return "<div id='" + (id == null || typeof id == 'undefined' ? "paymentmethodsPopup" : id) + 
...
PaymentMethodsPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='paymentmethodsEverything' href='javascript:void(0);'>", lang.payment_methods_everything, "</a></div>");
    }
    return ar;
}

PaymentMethodsPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

PaymentMethodsPopup.prototype._onsearch = function(value) {
...
PaymentMethodsPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].payment_method_id);
    }
}

PaymentMethodsPopup.prototype._oneverything = function() {
...
PaymentMethodsPopup.prototype._show = function(arg, offset) {
...
PaymentMethodsPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

PaymentMethodsPopup.prototype.hide = function() {
    this._container.hide();
}

PaymentMethodsPopup.prototype.toggle = function(arg, offset) {
