...
function ConfirmationTypesPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof ConfirmationTypesPopup) ) {
	return new ConfirmationTypesPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("confirmationtypesPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("confirmationtypesPopup");
    }
	...
	if( arg.getAttribute('name') == 'confirmationtypesEverything' ) {
...
(function (ConfirmationTypesPopup, undefined) {
    ConfirmationTypesPopup.container = function(id) {
	return "<div id='" + (id == null || typeof id == 'undefined' ? "confirmationtypesPopup" : id) + 
	    "' class='ballon'><div class='arrow'></div><div class='body' style='min-height: 30px;'></div></div>";
    };
}(ConfirmationTypesPopup));


/** private functions: **/

ConfirmationTypesPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='confirmationtypesEverything' href='javascript:void(0);'>", lang.confirmations.everything, "</a></div>");
    }
    return ar;
}

ConfirmationTypesPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

ConfirmationTypesPopup.prototype._onsearch = function(value) {
...
ConfirmationTypesPopup.prototype._onselect = function(index) {
...
ConfirmationTypesPopup.prototype._oneverything = function() {
...
ConfirmationTypesPopup.prototype._show = function(arg, offset) {
...
ConfirmationTypesPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

ConfirmationTypesPopup.prototype.hide = function() {
    this._container.hide();
}

ConfirmationTypesPopup.prototype.toggle = function(arg, offset) {
