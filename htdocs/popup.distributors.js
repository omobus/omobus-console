...
function DistributorsPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof DistributorsPopup) ) {
	return new DistributorsPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("distributorsPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("distributorsPopup");
    }
	...
	if( arg.getAttribute('name') == 'distributorsEverything' ) {
...
DistributorsPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='distributorsEverything' href='javascript:void(0);'>", lang.distributors_everything, "</a></div>");
    }
    return ar;
}

DistributorsPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

DistributorsPopup.prototype._onsearch = function(value) {
...
DistributorsPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].distributor_id);
    }
}

DistributorsPopup.prototype._oneverything = function() {
...
DistributorsPopup.prototype._show = function(arg, offset) {
...
DistributorsPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

DistributorsPopup.prototype.hide = function() {
    this._container.hide();
}

DistributorsPopup.prototype.toggle = function(arg, offset) {
