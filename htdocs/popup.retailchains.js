...
function RetailChainsPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof RetailChainsPopup) ) {
	return new RetailChainsPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("retailchainsPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("retailchainsPopup");
    }
	...
	if( arg.getAttribute('name') == 'retailchainsEverything' ) {
...
(function (RetailChainsPopup, undefined) {
    RetailChainsPopup.container = function(id) {
	var ar = [];
	ar.push("<div id='", id == null || typeof id == 'undefined' ? "retailchainsPopup" : id, "' class='ballon'>");
...
RetailChainsPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='retailchainsEverything' href='javascript:void(0);'>", lang.rc_everything, "</a></div>");
    }
    return ar;
}

RetailChainsPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

RetailChainsPopup.prototype._onsearch = function(value) {
...
RetailChainsPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].rc_id);
    }
}
...
RetailChainsPopup.prototype._show = function(arg, offset) {
...
RetailChainsPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

RetailChainsPopup.prototype.hide = function() {
    this._container.hide();
}

RetailChainsPopup.prototype.toggle = function(arg, offset) {
