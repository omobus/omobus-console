...
function PotentialsPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof PotentialsPopup) ) {
	return new PotentialsPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("potentialsPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("potentialsPopup");
    }
	...
	if( arg.getAttribute('name') == 'potentialsEverything' ) {
...
(function (PotentialsPopup, undefined) {
    PotentialsPopup.container = function(id) {
	return "<div id='" + (id == null || typeof id == 'undefined' ? "potentialsPopup" : id) + 
...
PotentialsPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='potentialsEverything' href='javascript:void(0);'>", lang.poten_everything, "</a></div>");
    }
    return ar;
}

PotentialsPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

PotentialsPopup.prototype._onsearch = function(value) {
...
PotentialsPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].poten_id);
    }
}

PotentialsPopup.prototype._oneverything = function() {
...
PotentialsPopup.prototype._show = function(arg, offset) {
...
PotentialsPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

PotentialsPopup.prototype.hide = function() {
    this._container.hide();
}

PotentialsPopup.prototype.toggle = function(arg, offset) {
