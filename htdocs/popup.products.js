...
function ProductsPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof ProductsPopup) ) {
	return new ProductsPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("productsPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("productsPopup");
    }
	...
	if( arg.getAttribute('name') == 'productsEverything' ) {
...
(function (ProductsPopup, undefined) {
    ProductsPopup.container = function(id) {
	return "<div id='" + (id == null || typeof id == 'undefined' ? "productsPopup" : id) + 
...
ProductsPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    for( var i = 0, size = rows.length; i < size; i++ ) {
	    ...
	    if( !String.isEmpty(r.code) ) {
		ar.push("<br/><span class='watermark'>", lang.p_code, ": ", G.shielding(r.code, lang.dash), "</span>");
	    }
	    ar.push("</td>");
	    ar.push("</tr>");
        }
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='productsEverything' href='javascript:void(0);'>", lang.prod_everything, "</a></div>");
    }
    return ar;
}

ProductsPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

ProductsPopup.prototype._onsearch = function(value) {
...
ProductsPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].prod_id);
    }
}

ProductsPopup.prototype._oneverything = function() {
...
ProductsPopup.prototype._show = function(arg, offset) {
...
ProductsPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

ProductsPopup.prototype.hide = function() {
    this._container.hide();
}

ProductsPopup.prototype.toggle = function(arg, offset) {
    if( this._container.toggle() ) {
	this._show(arg, offset);
    }
}

ProductsPopup.prototype.isHidden = function() {
