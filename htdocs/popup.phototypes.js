...
function PhotoTypesPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof PhotoTypesPopup) ) {
	return new PhotoTypesPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("phototypesPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("phototypesPopup");
    }
	...
	if( arg.getAttribute('name') == 'phototypesEverything' ) {
...
PhotoTypesPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='phototypesEverything' href='javascript:void(0);'>", lang.photos.everything, "</a></div>");
    }
    return ar;
}

PhotoTypesPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

PhotoTypesPopup.prototype._onsearch = function(value) {
...
PhotoTypesPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].photo_type_id);
    }
}

PhotoTypesPopup.prototype._oneverything = function() {
...
PhotoTypesPopup.prototype._show = function(arg, offset) {
...
PhotoTypesPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

PhotoTypesPopup.prototype.hide = function() {
    this._container.hide();
}

PhotoTypesPopup.prototype.toggle = function(arg, offset) {
