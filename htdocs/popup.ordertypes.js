...
function OrderTypesPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof OrderTypesPopup) ) {
	return new OrderTypesPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("ordertypesPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("ordertypesPopup");
    }
	...
	if( arg.getAttribute('name') == 'ordertypesEverything' ) {
...
(function (OrderTypesPopup, undefined) {
    OrderTypesPopup.container = function(id) {
	return "<div id='" + (id == null || typeof id == 'undefined' ? "ordertypesPopup" : id) + 
...
OrderTypesPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='ordertypesEverything' href='javascript:void(0);'>", lang.order_types_everything, "</a></div>");
    }
    return ar;
}

OrderTypesPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

OrderTypesPopup.prototype._onsearch = function(value) {
...
OrderTypesPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].order_type_id);
    }
}

OrderTypesPopup.prototype._oneverything = function() {
...
OrderTypesPopup.prototype._show = function(arg, offset) {
...
OrderTypesPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

OrderTypesPopup.prototype.hide = function() {
    this._container.hide();
}

OrderTypesPopup.prototype.toggle = function(arg, offset) {
    if( this._container.toggle() ) {
	this._show(arg, offset);
    }
}

OrderTypesPopup.prototype.isHidden = function() {
