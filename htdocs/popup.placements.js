...
function PlacementsPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof PlacementsPopup) ) {
	return new PlacementsPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("placementsPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("placementsPopup");
    }
	...
	if( arg.getAttribute('name') == 'placementsEverything' ) {
...
PlacementsPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='placementsEverything' href='javascript:void(0);'>", lang.placement_everything, "</a></div>");
    }
    return ar;
}

PlacementsPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

PlacementsPopup.prototype._onsearch = function(value) {
...
PlacementsPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].placement_id);
    }
}

PlacementsPopup.prototype._oneverything = function() {
...
PlacementsPopup.prototype._show = function(arg, offset) {
...
PlacementsPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

PlacementsPopup.prototype.hide = function() {
    this._container.hide();
}

PlacementsPopup.prototype.toggle = function(arg, offset) {
