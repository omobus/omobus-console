...
function CommentTypesPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof CommentTypesPopup) ) {
	return new CommentTypesPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("commenttypesPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("commenttypesPopup");
    }
	...
	if( arg.getAttribute('name') == 'commenttypesEverything' ) {
...
CommentTypesPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='commenttypesEverything' href='javascript:void(0);'>", lang.comments.everything, "</a></div>");
    }
    return ar;
}

CommentTypesPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

CommentTypesPopup.prototype._onsearch = function(value) {
...
CommentTypesPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].comment_type_id);
    }
}

CommentTypesPopup.prototype._oneverything = function() {
...
CommentTypesPopup.prototype._show = function(arg, offset) {
...
CommentTypesPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

CommentTypesPopup.prototype.hide = function() {
    this._container.hide();
}

CommentTypesPopup.prototype.toggle = function(arg, offset) {
