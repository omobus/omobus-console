...
function DeliveryTypesPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof DeliveryTypesPopup) ) {
	return new DeliveryTypesPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("deliverytypesPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("deliverytypesPopup");
    }
	...
	if( arg.getAttribute('name') == 'deliverytypesEverything' ) {
...
(function (DeliveryTypesPopup, undefined) {
    DeliveryTypesPopup.container = function(id) {
	return "<div id='" + (id == null || typeof id == 'undefined' ? "deliverytypesPopup" : id) + 
...
DeliveryTypesPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='deliverytypesEverything' href='javascript:void(0);'>", lang.delivery_types_everything, "</a></div>");
    }
    return ar;
}

DeliveryTypesPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

DeliveryTypesPopup.prototype._onsearch = function(value) {
...
DeliveryTypesPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].delivery_type_id);
    }
}

DeliveryTypesPopup.prototype._oneverything = function() {
...
DeliveryTypesPopup.prototype._show = function(arg, offset) {
...
DeliveryTypesPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

DeliveryTypesPopup.prototype.hide = function() {
    this._container.hide();
}

DeliveryTypesPopup.prototype.toggle = function(arg, offset) {
