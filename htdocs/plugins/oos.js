...
var PLUG = (function() {
	...
	ar.push("<span>", lang.oos.title1, "</span>&nbsp;");
	...
	ar.push(OOSTypesPopup.container());
	...
	for( var i = 0, k = 0; i < size; i++ ) {
		...
		if( (page-1)*perm.rows <= x && x < page*perm.rows ) {
		    ...
		    ar.push("<td class='ref sw95px'>", G.shielding(r.oos_type), "</td>");
	...
	var func = function(data_ts, ar, templ) {
		...
		.then(wb => {
		    ...
		    wb.property('Title', lang.oos.title);
			...
			ws.cell("R{0}".format_a(i + offset)).value(r.oos_type);
			ws.cell("S{0}".format_a(i + offset)).value(r.plu_code);
			ws.cell("T{0}".format_a(i + offset)).value(r.note);
...
/* public properties & methods */
	...
	types: function(tag, offset) {
	    _togglePopup("oos_types", tag, offset, function(obj) {
		return OOSTypesPopup(_cache.data[obj], function(arg, i, ar) {
		    _onpopup(tag, arg, "oos_type_id");
		})
	    });
	},
	xlsx: function() {
