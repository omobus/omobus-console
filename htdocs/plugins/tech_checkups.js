...
PLUG.registerRef("checkup", (function() {
    ...
    function _getstatus(exist) {
	if( exist == 1 ) return lang.plus;
	if( exist == 2 ) return lang.shortage;
	return lang.dash;
    }
	...
	for( var i = 0, size = Array.isArray(data.rows) ? data.rows.length : 0; i < size; i++ ) {
	    ...
	    ar.push("<td class='clickable autoincrement' onclick='PLUG.getRef(\"checkup\").checkrow(this.parentNode,\"",
		k, "\")'>", r.row_no, "</td>");
    ...
    return {
	getcaption: function() { return lang.doctypes.checkup; },
	...
	setdata: function(body, user_id, date, cb) {
		...
		G.xhr("GET", G.getdataref({plug: "tech", code: "tech_checkups", user_id: user_id, date: G.getdate(date)}), "json", function(xhr, data) {
