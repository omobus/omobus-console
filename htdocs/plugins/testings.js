...
var PLUG = (function() {
	...
	ar.push("<span>", lang.testings.title1, "</span>&nbsp;");
	...
	return Filter(a.join(' '), false, {
	    ...
	    chan_id:true, chan:true, poten:true, 
	    rc_id:true, rc:true, ka_code:true, 
	    region:true, 
	    city:true, 
	    contact_id:true, name:true, surname:true, 
	...
	ar.push("<div><b>", _fmtcontact(r), "</b></div>");
	ar.push("<div>", G.shielding(r.job_title), "</div>");
	ar.push("<div>", "{0}: <b>{1}</b>".format_a(lang.sla.result, G.getpercent_l(r.sla)), "</div>");
	...
	var func = function(data_ts, ar, templ) {
		...
		.then(wb => {
			...
			ws.cell("N{0}".format_a(i + offset)).value(_fmtcontact(r));
			ws.cell("O{0}".format_a(i + offset)).value(r.job_title);
			ws.cell("P{0}".format_a(i + offset)).value(G.getpercent_l(r.sla));
...
/* public properties & methods */
	...
	more: function(row_no) {
	    Dialog({width: 700, title: lang.sla.ratings, body: _detailsbody(_cache.data.rows[row_no-1])}).show();
	},
	xlsx: function() {
	    _toxlsx();
	}
