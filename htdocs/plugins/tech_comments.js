...
PLUG.registerRef("comment", (function() {
	...
	ar.push("<th>", lang.comments.type, "</th>");
	...
	for( var i = 0, size = Array.isArray(data.rows) ? data.rows.length : 0; i < size; i++ ) {
	    ...
	    ar.push("<td class='clickable autoincrement' onclick='PLUG.getRef(\"comment\").checkrow(this.parentNode,\"", 
		k, "\")'>", r.row_no, "</td>");
	    ...
	    ar.push("<td class='ref'>", G.shielding(r.comment_type), "</td>");
    ...
    return {
	getcaption: function() { return lang.doctypes.comment; },
	...
	setdata: function(body, user_id, date, cb) {
		...
		G.xhr("GET", G.getdataref({plug: "tech", code: "tech_comments", user_id: user_id, date: G.getdate(date)}), "json", function(xhr, data) {
