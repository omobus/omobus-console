...
var PLUG = (function() {
	...
	for( var i = 0, size = Array.isArray(r.brands) ? r.brands.length : 0; i < size; i++ ) {
	    x = r.brands[i];
	    ar.push("<tr>");
	...
	for( var i = 0; i < size; i++ ) {
		...
		if( (page-1)*perm.rows <= x && x < page*perm.rows ) {
			...
			ar.push("<td class='int'><a class='attention footnote_L' data-title='", lang.shelfs.warn.format_a(G.getpercent_l(r.sos_target), 
			    G.getpercent_l((r.sos*100.0/r.sos_target).toFixed(1))), "' href='javascript:void(0)' onclick='PLUG.more(", r.row_no, ");'>", 
		    ...
		    if( r.soa == null ) {
			ar.push("<td class='int'>", lang.dash, "</td>");
		    } else if( r.soa_target != null && r.soa_target > r.soa ) {
			ar.push("<td class='int'><a class='attention footnote_L' data-title='", lang.shelfs.warn.format_a(G.getpercent_l(r.soa_target), 
			    G.getpercent_l((r.soa*100.0/r.soa_target).toFixed(1))), "' href='javascript:void(0)' onclick='PLUG.more(", r.row_no, ");'>", 
			    G.getpercent_l(r.soa), "</a></td>");
		    } else {
			ar.push("<td class='int'><a href='javascript:void(0)' onclick='PLUG.more(" + r.row_no + ");'>", 
			    G.getpercent_l(r.soa), "</a></td>");
		    }
		    ar.push("<td class='ref" + (perm.columns != null && perm.columns.head == true ? " delim" : "") + "'>");
	...
	var func = function(data_ts, ar, templ) {
		...
		.then(wb => {
			...
			ws.cell("O{0}".format_a(i + offset)).value(G.getpercent_l(r.sos));
			if( r.sos_target != null ) {
			    ws.cell("P{0}".format_a(i + offset)).value(G.getpercent_l(r.sos_target));
			    ws.cell("Q{0}".format_a(i + offset)).value(G.getpercent_l((r.sos*100.0/r.sos_target).toFixed(1)));
			}
			ws.cell("R{0}".format_a(i + offset)).value(G.getpercent_l(r.soa));
			if( r.soa_target != null ) {
			    ws.cell("S{0}".format_a(i + offset)).value(G.getpercent_l(r.soa_target));
			    ws.cell("T{0}".format_a(i + offset)).value(G.getpercent_l((r.soa*100.0/r.soa_target).toFixed(1)));
			}
...
/* public properties & methods */
	...
	more: function(row_no) {
	    Dialog({width: 550, title: lang.shelfs.title, body: _detailsbody(_cache.data.rows[row_no-1])}).show();
	},
