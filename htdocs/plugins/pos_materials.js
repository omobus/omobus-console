...
/* private properties & methods */
	...
	for( var i = 0; i < size; i++ ) {
		...
		if( (page-1)*perm.rows <= x && x < page*perm.rows ) {
			...
			ar.push("<a href='", G.getdataref({plug: _code, posm_id: r.posm_id, blob: true}), "' target='_blank'>",
			    r.blob_size ? G.getnumeric_l(r.blob_size/1024, 1) : lang.dash, "</a>");
		    }
		    ar.push("</td>");
			...
			Array.prototype.push.apply(ar, _shieldingStringArray(r.placements));
		    } else {
...
/* public properties & methods */
	...
	slideshow: function(arg) {
	    SlideshowSimple([G.getdataref({plug: _code, blob: "yes", posm_id: arg})]).show();
	},
	drop: function(files) {
	    if( _perm.add ) {
	...
	edit: function(arg) {
		...
		commitView.onclick = function() {
			...
			G.xhr("PUT", G.getdataref({plug: _code, posm_id: data.posm_id}), "json", function(xhr, resp) {
			    if( xhr.status == 200 ) {
				PLUG.refresh();
				dialogObject.hide();
			    } else {
	...
	unlink: function(tag, posm_id) {
	    ...
	    G.xhr("DELETE", G.getdataref({plug: _code, posm_id: posm_id}), "", function(xhr) {
		if( xhr.status == 200 ) {
	...
	placements: function(tag, offset) {
		...
		return PlacementsPopup(_cache.data._f[obj], function(arg, i, ar) {
		    _onpopup(tag, arg, "placement_id", "placement_ids");
		})
	    });
	},
