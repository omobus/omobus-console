...
var PLUG = (function() {
	...
	var func = function(data_ts, ar, templ) {
		...
		.then(wb => {
			...
			ws.cell("N{0}".format_a(i + offset)).value(r.distributor);
			ws.cell("O{0}".format_a(i + offset)).value(r.warehouse);
			ws.cell("P{0}".format_a(i + offset)).value(Date.parseISO8601(r.delivery_date));
			ws.cell("Q{0}".format_a(i + offset)).value(r.delivery_type);
			ws.cell("R{0}".format_a(i + offset)).value(r.delivery_note);
			ws.cell("S{0}".format_a(i + offset)).value(r.payment_method);
			ws.cell("T{0}".format_a(i + offset)).value(r.payment_delay);
			ws.cell("U{0}".format_a(i + offset)).value(r.amount);
