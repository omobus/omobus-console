...
PLUG.registerRef("presence", (function() {
	...
	for( var i = 0, size = Array.isArray(data.rows) ? data.rows.length : 0; i < size; i++ ) {
	    ...
	    ar.push("<td class='clickable autoincrement' onclick='PLUG.getRef(\"presence\").checkrow(this.parentNode,\"",
		k, "\")'>", r.row_no, "</td>");
	    ...
	    ar.push("<td class='int'>", G.getint_l(r.facing), "</td>");
    ...
    return {
	getcaption: function() { return lang.doctypes.presence; },
	...
	setdata: function(body, user_id, date, cb) {
		...
		G.xhr("GET", G.getdataref({plug: "tech", code: "tech_presences", user_id: user_id, date: G.getdate(date)}), "json", function(xhr, data) {
