...
var PLUG = (function() {
	...
	for( var i = 0, x = 0, size = rows == null ? 0 : rows.length; i < size; i++ ) {
		...
		ar.push({ref: G.getdataref({plug: _code, blob: "yes", blob_id: r.blob_id}), explain: _getexplain(r, perm).join("")});
		x++;
	    }
	}
	...
	if( String.isEmpty(params.sub) ) {
	    ...
	    var xhr = G.xhr("POST", G.getdataref({plug: _code, doc_id: doc_id}), "", function(xhr) {
		if( xhr.status == 200 ) {
		    params.sub = null;
		    params.msg = null;
		    params.strict = null;
	...
	var func = function(data_ts, ar, templ) {
		...
		.then(wb => {
		    ...
		    wb.property('Title', lang.posms.title);
