...
var PLUG = (function() {
	...
	var func = function(data_ts, ar, templ) {
		...
		.then(wb => {
		    ...
		    wb.property('Title', lang.reclamations.title);
			...
			ws.cell("O{0}".format_a(i + offset)).value(Date.parseISO8601(r.return_date));
			ws.cell("P{0}".format_a(i + offset)).value(r.amount);
			ws.cell("Q{0}".format_a(i + offset)).value(r.doc_note);
			ws.cell("R{0}".format_a(i + offset)).value(r.head_name);
		    }
		    wb.outputAsync()
			.then(function(blob) {
