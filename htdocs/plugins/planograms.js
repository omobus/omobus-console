...
/* private properties & methods */
	...
	return "<table class='headerbar' width='100%'><tr><td><h1>" + lang.planograms.title + ":&nbsp;&nbsp;" +
	    ...
	    (perm.add != null?("&nbsp&nbsp;|&nbsp;&nbsp;<a href='javascript:PLUG.add();'>" + lang.planograms.add + "</a>"):"") +
	    "</td></tr></table>" +
	    "<table width='100%' class='report'><thead><tr>" + 
	    "<th class='autoincrement'>" + lang.num + "</th>" + 
	    "<th>" + lang.planogram + "</th>" + 
	    "<th>" + lang.code + "</th>" + 
	    "<th>" + lang.planograms.blob + "</th>" + 
	    "<th>" + lang.country + "</th>" + 
	    "<th>" + lang.brand + "</th>" + 
	...
	for( var i = 0, x = 1, size = rows ? rows.length : 0; i < size; i++ ) {
		...
		ar.push("<tr" + (objcache.getchecked(r.pl_id) ? " class='selected'" : "") + ">");
		ar.push("<td style='cursor:pointer' class='autoincrement' onclick=\"PLUG.oncheckrow(this.parentNode,'" + r.pl_id +
		    "');event.stopPropagation();\">" + r.row_no + "</td>");
		ar.push("<td class='string'>" +
		    (r.owner||perm&&perm.edit ? "<a href='javascript:PLUG.edit(" + r.row_no + ",\"" + r.pl_id + "\");'>" : "") +
		    r.descr +
		    (r.owner||perm&&perm.edit ? "</a>" : "") + "</td>");
		ar.push("<td class='int'>" + r.pl_id + "</td>");
		ar.push("<td class='int' width='60px;'><a class='group1' href='" + G.getajax({plug: _code, pl_id: r.pl_id, blob: true}) + "' title='" + r.descr + "'>" + 
		...
		ar.push("<td class='ref' width='200px;'>" + G.shielding(r.channels) + "</td>");
		...
		if( r.hidden ) {
		    ...
		    ar.push("<a href='javascript:PLUG.remove(" + r.row_no + ",\"" + r.pl_id + "\");'>" + lang.remove.ref + "</a>");
		    ar.push("</td>");
		}
    ...
    function _remove(tbody, row_no, pl_id) {
	...
	$.each(files, function(i, f) {
		...
		Toast.show(lang.planograms.msg1.format_a(f.name, perm.add.max_file_size_mb));
	    } else {
		_storeBlob({
		    _file: f,
		    _data: _cache,
		    name: f.name.replace(/\.[^/.]+$/, ""),
		    channels: _cloneArray(_cache.channels, null, function(r) {return r.chan_id;})
		});
	    }
	});
    }
	...
	_context = {
	    ...
	    channels: _cloneArray(_cache.channels, row.chan_ids, function(r) {return r.chan_id;})
	};
	_showbox(_context, _elem.box, _getbox(lang.planograms.caption1.format_a(_context.pl_id), _context));
    }
	...
	a.push("<div class='row'>" + lang.planograms.notice + "</div>");
	...
	a.push("<div class='row'><input type='text' maxlength='192' autocomplete='off' oninput='PLUG.e.caption(this.value)' placeholder='" +
	    lang.planograms.placeholder + "' value='" + context.name + "'/></div>");
	...
	if( context._data.channels ) {
	    a.push("<div class='row'>" + lang.chan_name + ":&nbsp;<a id='channel' href='javascript:PLUG.e.channel();'>" +
	...
	a.push("<div align='right'><button onclick='PLUG.hidebox();;event.stopPropagation();'>" + lang.back + "</button>&nbsp;&nbsp;<button id='commit' " +
	    "onclick='PLUG.save();;event.stopPropagation();' " + (context.pl_id?"disabled='true'":"") + ">" + lang.save + "</button></div>");
	...
	_context = context;
	_showbox(_context, _elem.box, _getbox(lang.planograms.caption0, _context));
    }
	...
	if( context.name.isEmpty() ) {
	    _showmsg(_context._msg, lang.planograms.msg2);
	} else if( context.b_date > context.e_date ) {
	    _showmsg(_context._msg, lang.planograms.msg3);
	} else {
	    var fd, sp, xhr, t;
	    fd = new FormData();
	    if( context.pl_id ) fd.append("pl_id", context.pl_id);
	    ...
	    if( (t = _fmtArray(context.channels, "chan_id")) != null ) fd.append("chan_ids", t);
	    _hidemsg(_context._msg);
	    sp = new Spinner(_opt.D).spin(_context._spin.get(0));
	    xhr = G.xhr(context.pl_id ? "PUT" : "POST", G.getajax({plug: _code, pl_id: context.pl_id}), "", function(xhr) {
...
/* public properties & methods */
	...
	remove: function(row_no, pl_id) {
	    if( _perm ) {
		_remove(_elem.tbody, row_no, pl_id);
	    }
	},
	add: function() {
	...
	e: {
	    ...
	    channel: function() {
		_showpopup(_context._popup, _elem.box.find("#channel"), _getmultilist(_context.channels, "channel"));
	    },
	    country: function() {
	    ...
	    _fchannel: function(value) {
		_filterML(value, _context.channels, _context._popup);
	    },
	    _chkchannel: function(i, checked) {
		_context.channels[i].checked = checked ? true : null;
		    _elem.box.find("#channel").text(_fmtArrayL(_context.channels, "chan_id"));
		_redrawbox(_context);
	    }
	}
    }
