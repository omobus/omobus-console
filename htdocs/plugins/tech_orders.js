...
PLUG.registerRef("order", (function() {
	...
	for( var i = 0, size = Array.isArray(data.rows) ? data.rows.length : 0; i < size; i++ ) {
	    ...
	    ar.push("<td class='clickable autoincrement' onclick='PLUG.getRef(\"order\").checkrow(this.parentNode,\"",
		k, "\")'>", r.row_no, "</td>");
	    ...
	    ar.push("<td class='string'>", G.shielding(r.order_type), "</td>");
	    ar.push("<td class='date'>", G.getdate_l(Date.parseISO8601(r.delivery_date)), "</td>");
	    ...
	    ar.push("<td class='int'>", r.discount > 0 ? G.getpercent_l(r.discount) : lang.dash, "</td>");
    ...
    return {
	getcaption: function() { return lang.doctypes.order; },
	...
	setdata: function(body, user_id, date, cb) {
		...
		G.xhr("GET", G.getdataref({plug: "tech", code: "tech_orders", user_id: user_id, date: G.getdate(date)}), "json", function(xhr, data) {
