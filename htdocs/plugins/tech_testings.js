...
PLUG.registerRef("testing", (function() {
	...
	for( var i = 0, size = Array.isArray(data.rows) ? data.rows.length : 0; i < size; i++ ) {
	    ...
	    ar.push("<td class='clickable autoincrement' onclick='PLUG.getRef(\"testing\").checkrow(this.parentNode,\"",
		k, "\")'>", r.row_no, "</td>");
	    ...
	    ar.push("<td class='int'><a href='javascript:void(0)' onclick='PLUG.getRef(\"testing\").more(", i, ");'>", 
    ...
    return {
	getcaption: function() { return lang.doctypes.testing; },
	...
	setdata: function(body, user_id, date, cb) {
		...
		G.xhr("GET", G.getajax({plug: "tech", code: "tech_testings", user_id: user_id, date: G.getdate(date)}), "json", function(xhr, data) {
