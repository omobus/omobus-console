...
var PLUG = (function() {
	...
	ar.push("<span>", lang.advt.title1, "</span>&nbsp;");
	...
	var func = function(data_ts, ar, templ) {
		...
		.then(wb => {
		    ...
		    wb.property('Title', lang.advt.title);
			...
			ws.cell("O{0}".format_a(i + offset)).value(r.posm_id);
			ws.cell("P{0}".format_a(i + offset)).value(r.posm);
