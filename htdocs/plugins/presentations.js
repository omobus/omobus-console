...
var PLUG = (function() {
	...
	ar.push("<span>", lang.presentations.title1, "</span>&nbsp;");
	...
	if( perm.columns != null && perm.columns.brand == true ) {
	    ar.push("<th class='sw95px'><a href='javascript:void(0)' onclick='PLUG.brands(this, 0.65)'>", lang.brand, "</a></th>");
	}
	ar.push("<th width='75px'>", lang.participants, "</th>");
	...
	for( var i = 0, k = 0; i < size; i++ ) {
		...
		if( (page-1)*perm.rows <= x && x < page*perm.rows ) {
		    ...
		    ar.push("<td class='string'>", G.shielding(r.tm), "</td>");
		    if( perm.columns != null && perm.columns.brand == true ) {
	...
	var func = function(data_ts, ar, templ) {
		...
		.then(wb => {
		    ...
		    wb.property('Title', lang.presentations.title);
			...
			ws.cell("Q{0}".format_a(i + offset)).value(r.participants);
