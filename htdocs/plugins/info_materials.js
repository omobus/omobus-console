...
/* private properties & methods */
	...
	ar.push("<span>", lang.info_materials.title, "</span>");
	...
	if( typeof perm.add != 'undefined' ) {
	    ar.push("&nbsp&nbsp;|&nbsp;&nbsp;<a href='javascript:void(0);' onclick='PLUG.add();'>", lang.info_materials.add, "</a>");
	}
	...
	for( var i = 0; i < size; i++ ) {
		...
		if( (page-1)*perm.rows <= x && x < page*perm.rows ) {
		    ar.push("<tr" + (typeof checked != 'undefined' && checked[r.infom_id] ? " class='selected'" : "") + ">");
		    ar.push("<td class='autoincrement clickable' onclick=\"PLUG.checkrow(this.parentNode,'" +
			r.infom_id + "');event.stopPropagation();\">", r.row_no, "</td>");
		    if( (perm.edit || r._isowner) && (typeof r._isaliendata == 'undefined' || r._isaliendata == 0) ) {
			ar.push("<td class='int'>", "<a href='javascript:void(0);' onclick='PLUG.edit(\"", r.infom_id, "\")'>", 
			    G.shielding(r.infom_id), "</a>", "</td>");
		    } else {
			ar.push("<td class='int'>", G.shielding(r.infom_id), "</td>");
		    }
			...
			ar.push("<a href='javascript:void(0);' onclick='PLUG.slideshow(\"", r.infom_id, "\")'>",
			    r.blob_size ? G.getnumeric_l(r.blob_size/1024, 1) : lang.dash, "</a>");
		    } else {
			ar.push("<a href='", G.getajax({plug: _code, infom_id: r.infom_id, blob: true}), "' target='_blank'>",
			    r.blob_size ? G.getnumeric_l(r.blob_size/1024, 1) : lang.dash, "</a>");
		    }
		    ar.push("</td>");
		    ar.push("<td class='ref sw95px", String.isEmpty(r.country_id) ? " incomplete" : "", "'>", G.shielding(r.country), "</td>");
		    ar.push("<td class='ref Xsw95px'>");
		    if( !String.isEmpty(r.department) ) {
			ar.push(G.shielding(r.department));
		    } else {
		    ...
		    ar.push("</td>");
		    ar.push("<td class='ref' Xclass='date'>");
			...
			ar.push("<td class='int'>", "<a href='javascript:void(0);' onclick='PLUG.unlink(this,\"", r.infom_id, "\")'>", 
			    lang.unlink, "</a>", "</td>");
		    } else {
...
/* public properties & methods */
	...
	slideshow: function(arg) {
	    SlideshowSimple([G.getajax({plug: _code, blob: "yes", infom_id: arg})]).show();
	},
	drop: function(files) {
	    if( _perm.add ) {
	...
	edit: function(arg) {
		...
		const data = _cache.data.rows.find(function(e) { return e.infom_id == arg; });
		const newData = {
		    descr: data.descr, 
		...
		if( !Array.isEmpty(mans.departments) ) {
		    depView.onclick = function(ev) {
			togglePopup("departments", depView, function(obj) {
			    return DepartmentsPopup(mans[obj], function(arg, i, ar) {
				newData.dep_id = typeof arg == 'object' ? arg.dep_id : null;
				depView.html(dropDownLabel(lang.department, arg));
				func();
			    })
			});
			ev.stopPropagation();
		...
		commitView.onclick = function() {
			...
			alertView.html(lang.info_materials.msg2);
			alertView.show();
		    } else if( String.isEmpty(newData.country_id) ) {
			alertView.html(lang.info_materials.msg3);
			alertView.show();
		    } else {
			let fd = new FormData();
			fd.append("_datetime", G.getdatetime(new Date()));
			...
			if( !String.isEmpty(newData.dep_id) ) {
			    fd.append("dep_id", newData.dep_id);
			}
			...
			G.xhr("PUT", G.getajax({plug: _code, infom_id: data.infom_id}), "json", function(xhr, resp) {
			    if( xhr.status == 200 ) {
				PLUG.refresh();
				dialogObject.hide();
			    } else {
