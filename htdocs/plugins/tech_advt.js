...
PLUG.registerRef("advt", (function() {
	...
	ar.push("<th>", lang.pos_materials, "</th>");
	...
	for( var i = 0, size = Array.isArray(data.rows) ? data.rows.length : 0; i < size; i++ ) {
	    ...
	    ar.push("<td class='clickable autoincrement' onclick='PLUG.getRef(\"advt\").checkrow(this.parentNode,\"",
		k, "\")'>", r.row_no, "</td>");
	    ...
	    ar.push("<td class='string'>", G.shielding(r.posm), "</td>");
    ...
    return {
	getcaption: function() { return lang.doctypes.advt; },
	...
	setdata: function(body, user_id, date, cb) {
		...
		G.xhr("GET", G.getdataref({plug: "tech", code: "tech_advt", user_id: user_id, date: G.getdate(date)}), "json", function(xhr, data) {
