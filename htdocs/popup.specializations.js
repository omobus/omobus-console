...
function SpecializationsPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof SpecializationsPopup) ) {
	return new SpecializationsPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("jobtitlesPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("jobtitlesPopup");
    }
	...
	if( arg.getAttribute('name') == 'jobtitlesEverything' ) {
...
(function (SpecializationsPopup, undefined) {
    SpecializationsPopup.container = function(id) {
	return "<div id='" + (id == null || typeof id == 'undefined' ? "jobtitlesPopup" : id) + 
	    "' class='ballon'><div class='arrow'></div><div class='body' style='min-height: 30px;'></div></div>";
    };
}(SpecializationsPopup));


/** private functions: **/

SpecializationsPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='jobtitlesEverything' href='javascript:void(0);'>", lang.specialization_everything, "</a></div>");
    }
    return ar;
}

SpecializationsPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

SpecializationsPopup.prototype._onsearch = function(value) {
...
SpecializationsPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].spec_id);
    }
}

SpecializationsPopup.prototype._oneverything = function() {
...
SpecializationsPopup.prototype._show = function(arg, offset) {
...
SpecializationsPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

SpecializationsPopup.prototype.hide = function() {
    this._container.hide();
}

SpecializationsPopup.prototype.toggle = function(arg, offset) {
    if( this._container.toggle() ) {
	this._show(arg, offset);
    }
}

SpecializationsPopup.prototype.isHidden = function() {
