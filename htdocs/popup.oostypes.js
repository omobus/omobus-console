...
function OOSTypesPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof OOSTypesPopup) ) {
	return new OOSTypesPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("oostypesPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("oostypesPopup");
    }
	...
	if( arg.getAttribute('name') == 'oostypesEverything' ) {
...
(function (OOSTypesPopup, undefined) {
    OOSTypesPopup.container = function(id) {
	return "<div id='" + (id == null || typeof id == 'undefined' ? "oostypesPopup" : id) + 
...
OOSTypesPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='oostypesEverything' href='javascript:void(0);'>", lang.oos_type_everything, "</a></div>");
    }
    return ar;
}

OOSTypesPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

OOSTypesPopup.prototype._onsearch = function(value) {
...
OOSTypesPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].oos_type_id);
    }
}

OOSTypesPopup.prototype._oneverything = function() {
...
OOSTypesPopup.prototype._show = function(arg, offset) {
...
OOSTypesPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

OOSTypesPopup.prototype.hide = function() {
    this._container.hide();
}

OOSTypesPopup.prototype.toggle = function(arg, offset) {
    if( this._container.toggle() ) {
	this._show(arg, offset);
    }
}

OOSTypesPopup.prototype.isHidden = function() {
