...
function DiscardTypesPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof DiscardTypesPopup) ) {
	return new DiscardTypesPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("discardtypesPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("discardtypesPopup");
    }
	...
	if( arg.getAttribute('name') == 'discardtypesEverything' ) {
...
DiscardTypesPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='discardtypesEverything' href='javascript:void(0);'>", lang.discard_type_everything, "</a></div>");
    }
    return ar;
}

DiscardTypesPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

DiscardTypesPopup.prototype._onsearch = function(value) {
...
DiscardTypesPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].discard_type_id);
    }
}

DiscardTypesPopup.prototype._oneverything = function() {
...
DiscardTypesPopup.prototype._show = function(arg, offset) {
...
DiscardTypesPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

DiscardTypesPopup.prototype.hide = function() {
    this._container.hide();
}

DiscardTypesPopup.prototype.toggle = function(arg, offset) {
    if( this._container.toggle() ) {
	this._show(arg, offset);
    }
}

DiscardTypesPopup.prototype.isHidden = function() {
