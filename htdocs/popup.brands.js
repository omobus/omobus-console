...
function BrandsPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof BrandsPopup) ) {
	return new BrandsPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("brandsPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("brandsPopup");
    }
	...
	if( arg.getAttribute('name') == 'brandsEverything' ) {
...
(function (BrandsPopup, undefined) {
    BrandsPopup.container = function(id) {
	return "<div id='" + (id == null || typeof id == 'undefined' ? "brandsPopup" : id) + 
...
BrandsPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    for( var i = 0, size = rows.length; i < size; i++ ) {
	    ...
	    if( typeof r.competitor != 'undefined' && r.competitor && !String.isEmpty(r.manuf) ) {
		ar.push("<br/><span class='watermark'>", G.shielding(r.manuf), "</span>");
	    }
	    ar.push("</td></tr>");
        }
    }
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='brandsEverything' href='javascript:void(0);'>", lang.brand_everything, "</a></div>");
    }
    return ar;
}

BrandsPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

BrandsPopup.prototype._onsearch = function(value) {
...
BrandsPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].brand_id);
    }
}

BrandsPopup.prototype._oneverything = function() {
...
BrandsPopup.prototype._show = function(arg, offset) {
...
BrandsPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

BrandsPopup.prototype.hide = function() {
    this._container.hide();
}

BrandsPopup.prototype.toggle = function(arg, offset) {
