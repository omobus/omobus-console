...
function QuestRowsPopup(rows, selection, params /* params = { everything: true|false, container = "DOM container"} */) {
    if( !(this instanceof QuestRowsPopup) ) {
	return new QuestRowsPopup(rows, selection, params);
    }
    if( params == null || typeof params == 'undefined' ) {
	params = { everything: true, container: _("qrowsPopup") };
    }
    if( params.container == null || typeof params.container == 'undefined' ) {
	params.container = _("qrowsPopup");
    }
	...
	if( arg.getAttribute('name') == 'qrowsEverything' ) {
...
QuestRowsPopup.prototype._get = function(rows, everything) {
    var ar = [], r;
    //if( rows.length > 5 ) {
    ...
    for( var i = 0, size = rows.length; i < size; i++ ) {
	    ...
	    ar.push("<td ", r.hidden ? "class='strikethrough'" : "", ">", G.shielding(r.descr, lang.dash));
	    if( !String.isEmpty(r.qpath) ) {
		ar.push("<br/><span class='watermark'>", G.shielding(r.qpath), "</span>");
	    }
	    if( !String.isEmpty(r.qname) ) {
		ar.push("<br/><span class='watermark'>", G.shielding(r.qname), "</span>");
	    }
	    ar.push("</td></tr>");
        }
    }
    ...
    if( everything ) {
	ar.push("<br />");
	ar.push("<div class='r'><a name='qrowsEverything' href='javascript:void(0);'>", lang.qrow_everything, "</a></div>");
    }
    return ar;
}

QuestRowsPopup.prototype._foreach = function(tags, cb) {
    for( var i = 0, size = tags == null ? 0 : tags.length; i < size; i++ ) {
	cb(tags[i], i);
    }
}

QuestRowsPopup.prototype._onsearch = function(value) {
...
QuestRowsPopup.prototype._onselect = function(index) {
	...
	this._container.setAttribute('X-uid', this._rows[index].qrow_id);
    }
}

QuestRowsPopup.prototype._oneverything = function() {
...
QuestRowsPopup.prototype._show = function(arg, offset) {
...
QuestRowsPopup.prototype.show = function(arg, offset) {
    if( this._container.isHidden() ) {
	this._show(arg, offset);
    }
}

QuestRowsPopup.prototype.hide = function() {
    this._container.hide();
}

QuestRowsPopup.prototype.toggle = function(arg, offset) {
